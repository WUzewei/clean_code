...
TEST_F(StudentMgrTest, insertNewStudent)
{
	...
	EXPECT_EQ(2, StudentMgr::getInstance().getAllStudents().size());
}

TEST_F(StudentMgrTest, searchStudent)
{
	uint32 student_id = 110;
	EXPECT_TRUE(StudentMgr::getInstance().searchStudentRecord(student_id, s4));
	EXPECT_STREQ("Alice", s4->getStudentName().c_str());

	student_id = 0;
	EXPECT_FALSE(StudentMgr::getInstance().searchStudentRecord(student_id, s4));
}
...
TEST_F(StudentMgrTest, updateStudent)
{
	...
	uint32 student_id = 110;
	StudentMgr::getInstance().searchStudentRecord(student_id, s1);
